...
/**
 * The thread responsible for performing all logical game operations; like
 * detecting collisions, calling move methods for various actors. Note: Nothing
 * in this thread should call draw.
 * 
 * @author dasm80x86
 */
public final class MainThread implements Runnable {
	...
	public synchronized void run() {
		...
		while (true) {
			...
			if (SnowPilot.mScreen != null && SnowPilot.mTerrain != null) {
				...
				if (!Terrain.isGenerated) {
					if (SnowPilot.mScreen.getWidth() > 0
						...
						SnowPilot.mTerrain = new Terrain(SnowPilot.mScreen
								.getHeight()
								- (SnowPilot.mScreen.getHeight() / 8),
								SnowPilot.mScreen);
				...
				if (SnowPilot.mRandom.nextInt(ONE_HUNDRED) < SNOW_THRESHOLD) {
					int tmpRandomX = SnowPilot.mRandom
							.nextInt(SnowPilot.mScreen.getWidth());
					int tmpRandomS = SnowPilot.mRandom.nextInt(10);
					SnowPilot.mSnow
							.add(new SnowFlake(tmpRandomX, 1, tmpRandomS));
				}
